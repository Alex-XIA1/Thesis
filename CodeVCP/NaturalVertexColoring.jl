...
"""
function NaturalColoringMILP(graph)
    ...
    @variable(lpModel, w[1:nbVertex], Bin)

    # sum of w_k
    @objective(lpModel, Min, sum(w[i] for i = 1:nbVertex))

    # Constraints 
    # constraints sum_k x_vk = 1 (max 1 color per vertex)
    for v in 1:nbVertex
        @constraint(lpModel, sum(x[v, k] for k in 1:nbVertex) == 1)
    end
...
"""
function NaturalColoringLP(graph)
    ...
    # variables w_k for each color
    @variable(lpModel, w[1:nbVertex] >= 0)
